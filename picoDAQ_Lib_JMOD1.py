# -*- coding: utf-8 -*-
"""
Created on Mon Apr 10 12:21:17 2023
...
def get_spi_vco():             
    ...
    try: 
        ...
        device = 1
        spi.open(bus, device)            
        spi.max_speed_hz = int(5e5)   #AD4351 has max frequency of clock as 10MHz (100ns period)#change to 10e5
        spi.mode = 0b00
        spi.lsbfirst = False
...
def read_block_singleOrMultiple_frames_at_unsynchronized_state_BIN_1Frame(spi_obj_arg,n_bytes_block_arg=1<<1,   flag_verbose = False, bufferMarker_List =[0xEFFF, 0xFFFF] ):
    ...
    bufferSignature_1D = 0 #-1*np.ones((nFrames,))
    flagSignatureFound_1D = False #np.array([False]*nFrames, dtype=bool)
    ...
    frames_data_nD = bytearray(2*2*N_rows*N_cols)

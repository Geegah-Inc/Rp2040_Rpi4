...
#savedirname ="/media/geegah/WD_BLACK/IMAGER_DATA/ANUJ/SNG10122023_PULSEWIDTHSWEEP_f1853_S25_P85_ADC2/"
...
print(len(time1))
time2 = [jj+80 for jj in time1]
# %%sweeping through time settings
...
for jj in range(steps):
    ...
    fname = rawdata_echo_dir + "frame"+str(start+jj*5)+".dat"
    picoDAQ_Lib.writeFile(fname, frames_data_nD_baseline)
    F_IMG_I, F_IMG_Q = picoDAQ_Lib.convertToIQImage(frames_data_nD_baseline)
    I_AE_VOLTS, Q_AE_VOLTS = picoDAQ_Lib.convertADCToVolts(F_IMG_I, F_IMG_Q)
...
# time how long this takes125
t1 = time.time()
mat_to_plot = np.array(Mag)-np.array(Magb)
for cf in range(len(Mag)):
    # read an image
    mytitle.set_text(base_title+"TIME: "+str(25+start+cf*5)+' ns')
    sub_image = np.flipud(np.rot90(mat_to_plot[cf], 1))
    pos201.set_data(sub_image)
    #pos201.set_clim(sub_image.min(), sub_image.max())
    pos201.set_clim(-0.03, 0.03)
    plt.pause(0.001)
...
plt.plot(time2[:-1], np.array(Mag_med) - np.array(Mag_medb), label="GAIN OF 5")
plt.title("Magnitude (V) vs sample and hold time")
plt.xlabel("Time (ns)")
plt.ylabel("Magnitude(V)")
plt.legend()
...
plt.plot(time2[:-1], np.array(pix1), label="Gain of 5")
plt.plot(time2[:-1], np.array(pix2), label="No GAIN")
